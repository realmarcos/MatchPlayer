...
/** Screen editMatchs */
function EditMatch({ navigation, route }: any) {
  ...
  useEffect(() => {
    async function fetchMatchs() {
      try {
        ...
        const hourStart = data.startHour.split(":")[0];
        const minuteStart = data.startHour.split(":")[1];
        const hourEnd = data.endHour.split(":")[0];
        const minuteEnd = data.endHour.split(":")[1];
        ...
        setTimeEnd(endTime);
        if (data.sportId === futebol) {
          setCheckFut(true);
        }
        if (data.sportId === volei) {
          setCheckFut(false);
          setCheckVol(true);
        }
        // setDate(new Date(data.day));
        // const start = new Date();
        // start.setHours(data.startHour);
        // console.log(+data.startHour.getHours());
      } catch (err: any) {
        showMessage({
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <Input
            ...
            value={match?.limitUsers?.toString()}
